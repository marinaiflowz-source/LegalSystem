        ...
        public async Task<IActionResult> GetAsync([FromQuery] BaseRefFilter filter)
        {
            ...
            dashboard.TeamPerformance = await (from ct in unitOfWork.CaseTeamRepository.GetAllQuerable().AsNoTracking()
                                               ...
                                               select new TeamPerformanceDto
                                               {
                                                   ...
                                                   LostCases = g.Count(x => x.ct.Case.ClosedStatus == CASEENUM.LOST),
            ...
            dashboard.UserCaseStatistics = await ( from caseItem in unitOfWork.CaseRepository.GetAllQuerable().AsNoTracking()
                                           join user in unitOfWork.UserRepository
                                           .GetAllQuerable()
                                           .AsNoTracking()
                                           on caseItem.AssignedUserId equals user.Id
                                                   group caseItem by new
                                                   {
                                                       UserId = user.Id,
            ...
            select new UserCaseStatisticsDto
            {
        UserId = (int)userCases.Key.UserId,
        UserName = userCases.Key.UserName,

        TotalCases = userCases.Count(),

        OpenCases = userCases.Count(x => !x.IsCompleted),

        ClosedCases = userCases.Count(x => x.IsCompleted),

        WonCases = userCases.Count(x =>
            x.IsCompleted &&
            x.ClosedStatus == CASEENUM.WON),

        SettledCases = userCases.Count(x =>
            x.IsCompleted &&
            x.ClosedStatus == CASEENUM.SETTLED),

        LostCases = userCases.Count(x =>
            x.IsCompleted &&
            x.ClosedStatus == CASEENUM.LOST),

        WinPercentage = userCases.Count(x => x.IsCompleted) == 0
            ? 0
            : Math.Round(
                (decimal)userCases.Count(x =>
                    x.IsCompleted &&
                    x.ClosedStatus == CASEENUM.WON)
                * 100 /
                userCases.Count(x => x.IsCompleted),
                2)
             })
